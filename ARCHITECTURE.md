...
│  └──────┘    └──┬───┘    └──┬───┘                            │
│                 │           │                                 │
│                 │           └──► BoltDB backend (MVCC data)  │
│                 ├──────────────► WAL (entries, hard state)   │
│                 └──────────────► Snapshotter ──► Snap store   │
└──────────────────────────────────────────────────────────────┘

┌──────────────────────────────────────────────────────────────┐
│ Client applications: Kubernetes API server, etcdctl, clients │
└──────────────────────────────────────────────────────────────┘
```

Each member has independent persistence paths. MVCC data is written to the
BoltDB backend, while Raft entries and hard state are written to the WAL.
Snapshotter-managed snapshot data is stored separately. During snapshot
handling, etcd saves the snapshot before recording the corresponding snapshot
metadata in the WAL.
...
**Backend Operations**:
```go
// Batch write from outside the Raft apply path
tx := be.BatchTx()
tx.LockOutsideApply()
defer tx.Unlock()
...
3. Snapshotter writes the snapshot file
4. Record snapshot metadata in the WAL after the snapshot is saved
...
_, err = cli.Put(putCtx, "key", "value")
putCancel()
...
// Get with prefix
prefixCtx, prefixCancel := context.WithTimeout(context.Background(), 5*time.Second)
resp, err = cli.Get(prefixCtx, "prefix", clientv3.WithPrefix())
prefixCancel()

// Delete
deleteCtx, deleteCancel := context.WithTimeout(context.Background(), 5*time.Second)
_, err = cli.Delete(deleteCtx, "key")
deleteCancel()
...
defer watchCancel()
watchChan := cli.Watch(watchCtx, "key")
for watchResp := range watchChan {
  for _, event := range watchResp.Events {
    fmt.Printf("Type: %s, ValueSize: %d\n",
      event.Type, len(event.Kv.Value))
  }
}
```

Do not log `event.Kv.Key` or `event.Kv.Value` directly. Etcd data may contain
customer data or secrets, so applications must redact keys and values before logging.

**Lease**:
```go
// Grant lease
grantCtx, grantCancel := context.WithTimeout(context.Background(), 5*time.Second)
lease, err := cli.Grant(grantCtx, 10) // 10 seconds
grantCancel()

// Put with lease
leasePutCtx, leasePutCancel := context.WithTimeout(context.Background(), 5*time.Second)
_, err = cli.Put(leasePutCtx, "key", "value", clientv3.WithLease(lease.ID))
leasePutCancel()
...
_, err = cli.Revoke(revokeCtx, lease.ID)
revokeCancel()
```
...
4. **Atomic**: Each changed key in a transaction generates its own event;
   events from one transaction share the same revision
...
Create the `root` user and grant it the `root` role before enabling
authentication. `auth enable` verifies these prerequisites; it does not create
the `root` user.
...
- Must exist before authentication is enabled
- Cannot be deleted
...
**Discovery Bootstrap**: Use a private etcd cluster as the discovery service
with v3 discovery. This procedure is supported in etcd v3.6 and later. The
legacy v2 protocol and public `discovery.etcd.io` service are not maintained.
```bash
# On the private discovery cluster, create a unique token and expected size.
token=$(uuidgen)
etcdctl --endpoints=http://discovery.example.com:2379 \
  put "/_etcd/registry/${token}/_config/size" 3

# Start each new member with the same token and private discovery endpoint.
etcd --name=member1 \
  --discovery-token="${token}" \
  --discovery-endpoints=http://discovery.example.com:2379
```

Repeat the member startup command with a unique name and peer configuration
for each member. See the [v3 discovery protocol documentation](https://etcd.io/docs/v3.6/dev-internal/discovery_protocol/)
for the discovery-cluster setup, authentication, and required peer settings.
...
**Database Size**:
- Default backend quota: 2 GiB
- Suggested maximum quota: 8 GiB
- Warning: logged when the configured quota exceeds 8 GiB
- `NOSPACE` alarm: raised when a write exceeds the configured quota
- Maximum tested: 100 GiB+
