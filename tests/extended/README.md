...
This repository contains the tests for the OpenShift Etcd.
...
| `./etcd-tests-ext list` | Lists all available test cases. |
| `./etcd-tests-ext run-suite <suite-name>` | Runs a test suite. e.g., `openshift/etcd/conformance/parallel` |
| `./etcd-tests-ext run-test <test-name>` | Runs one specific test. |

The tests can be run locally using the `etcd--tests-ext` binary against an OpenShift cluster.
Use the environment variable `KUBECONFIG` to point to your cluster configuration file such as:

```shell
export KUBECONFIG=path/to/kubeconfig
./etcd-tests-ext run-test <test-name>
```
...
3. Run the tests using the `etcd-tests-ext` binary.
...
```shell
./etcd-tests-ext run-suite openshift/etcd/conformance/parallel --junit-path $(ARTIFACT_DIR)/junit_$(shell date +%Y%m%d-%H%M%S).xml
```
...
|------------|-------------|
| `openshift/etcd/conformance/parallel` | Parallel conformance tests |
| `openshift/etcd/conformance/serial` | Serial conformance tests |
| `openshift/etcd/optional/slow` | Optional slow tests |
| `openshift/etcd/all` | All tests |
...
- as: tests-extension
  commands: |
    echo "Build binary etcd-tests-ext"
    make tests-ext-build
    echo "Running ./etcd-tests-ext with sanity test"
    ./etcd-tests-ext run-suite \
...
| Target | Description |
|--------|-------------|
| `make build` | Builds the etcd binary. |
| `make tests-ext-build` | Builds the test extension binary. |


## FAQ

### Why don't we have a Dockerfile for `etcd-tests-ext`?

We do not provide a Dockerfile for `etcd-tests-ext` because building and shipping a
...
The OTE binary (`etcd-tests-ext`) provides:
